...
FROM DUAL; -- INDEX번호 아님, 1부터 시작하는 COUNT정도

-- 문자열에서 5번째부터 시작해서 처음찾은 'B'의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA', 'B', 5)
FROM DUAL; -- 9

-- 문자열에서 5번째부터 시작해서 2번째로 찾은 'B'의 위치 조회
...
-- 이메일 아이디 오름차순으로 조회
SELECT EMP_NAME, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') -1) ID
FROM EMPLOYEE
ORDER BY ID ASC;
...
-- CEIL(숫자 | 컬럼명)  : 올림(씰)
-- FLOOR(숫자 | 컬럼명) : 내림
SELECT CEIL(3.14), FLOOR(8.65) FROM DUAL; -- 4,8
...
SELECT 123.456,
	TRUNC(123.456),	  	-- 123
	TRUNC(123.456, 1),	-- 123.4	(둘째자리부터 버림)
	TRUNC(123.456, -1)	-- 120
...
FROM DUAL; -- 1(달)
SELECT 
	MONTHS_BETWEEN( TO_DATE('2024-10-16', 'YYYY-MM-DD'),
	CURRENT_DATE)
FROM DUAL; -- 2(달)
...
FROM DUAL; -- 5.42(달) == 5개월 13일정도

/* MONTH_BETWEEN이 지정된 두 날짜사이의 차를 계산할때 훨씬 정확함
 * 
 * 달마다 일자가 다른데(28,29,30,31)
 * MONTH_BETWEEN은 이런 부분까지 모두 자동으로 적용되어 계산된다
 *  */

-- EMPLOYEE 테이블에서
-- 모든 사원의 이름, 입사일, 연차 조회하기
SELECT 
	EMP_NAME,
	HIRE_DATE,
	CEIL(MONTHS_BETWEEN( CURRENT_DATE, HIRE_DATE )/12) || '년차'
FROM EMPLOYEE;
...
/* 숫자 -> 문자열 변환시 발생할 수 있는 문제상환
 * 
 * -> 지정된 포멧의 칸 수가 변환하려는 숫자 자릿수 보다 적은경우
 * --> 모든 숫자가 #으로 변환되서 출력(데이터깨짐)
 */
...
SELECT 1234, TO_CHAR(1234,'999,999,999') 
FROM DUAL; -- '1,234'
...
SELECT CURRENT_DATE	-- 2024-08-16 14:06:57.000
FROM DUAL;
SELECT TO_CHAR(CURRENT_DATE, 'YYYY/MM/DD')	-- 2024/08/16
FROM DUAL;
SELECT TO_CHAR(CURRENT_DATE, 'MM/YYYY/DD')	-- 08/2024/16
FROM DUAL;

-- '2024-08-16 금요일'
SELECT TO_CHAR(CURRENT_DATE, 'YYYY-MM-DD DAY')	-- 2024-08-16 금요일
FROM DUAL;

-- '2024.08.16 (금) 오후 14:10:12'
SELECT TO_CHAR(CURRENT_DATE, 'YYYY.MM.DD (DY) AM HH24:MI:SS' )	--2024.08.16 (금) 오후 14:12:06
FROM DUAL;
SELECT TO_CHAR(CURRENT_DATE, 'YYYY.MM.DD (DY) PM! HH24:MI:SS' )	--2024.08.16 (금) 오후! 14:12:06
FROM DUAL;


-- '24년 8월 16일 금요일 오후 2시 15분 30초'
SELECT TO_CHAR(CURRENT_DATE, 'YY"년" MM"월" DD"일" DY"요일" AM HH"시" MI"분" SS"초"') 날짜
--SELECT TO_CHAR(CURRENT_DATE, 'YY년 MM월 DD일 DY요일 AM HH시 MI분 SS초')
FROM DUAL;
...
SELECT '2024-08-16' 문자열,
	TO_DATE('2024-08-16', 'YYYY-MM-DD') 날짜1,
	TO_DATE('2024-08-16') 날짜2
FROM DUAL;
-- TO_DATE 에 매개변수가 한개만 작성될 수 있는경우
		--> ()내부에 문자열이 "일반적인" 날짜/시간 형식일 때에만 가능

-- 일반적이지 않은경우
-- SELECT TO_DATE('16082024') 일반적이지 않은 경우 포멧 지정이 필
SELECT TO_DATE('16082024', 'DDMMYYYY')	--2024-08-16 00:00:00.000
FROM DUAL;

SELECT TO_DATE('24년 8월 16일 금요일 오후 2시 15분 30초',
	'YY"년" MM"월" DD"일" DY"요일" AM HH"시" MI"분" SS"초"') 날짜
FROM DUAL;	--2024-08-16 14:15:30.000
...
-- 50 미만 확인
SELECT 	TO_DATE('49-12-25', 'YY-MM-DD'),	--2049-12-25 00:00:00.000
				TO_DATE('49-12-25', 'RR-MM-DD')		--2049-12-25 00:00:00.000
FROM DUAL;

-- 50 이상 확인
SELECT 	TO_DATE('50-12-25', 'YY-MM-DD'),	--2050-12-25 00:00:00.000
				TO_DATE('50-12-25', 'RR-MM-DD')		--1950-12-25 00:00:00.000
FROM DUAL;
...
SELECT
	...
	SALARY 급여,
	BONUS 보너스,
--	SALARY*BONUS => NULL값 발생
	SALARY*NVL(BONUS, 0) 보너스지급액
FROM EMPLOYEE;
...
SELECT FLOOR( AVG(SALARY) )	--4091140
FROM EMPLOYEE;
...
---> TIP : SELECT 결과 집합 RESULT SET 이 찌그러지지않은 직각사각형 형태 일때만 조회 가능
...
-- EMPLOYEE 테이블의 모든 행의 갯수
SELECT COUNT(*)	-- 23
FROM EMPLOYEE;

-- EMPLOYEE 테이블의 부서코드가 'D5'인 사원의 숫자
SELECT	COUNT(*)	--	6명
FROM EMPLOYEE
WHERE DEPT_CODE='D5';

-- EMPLOYEE 테이블의 전화번호가 등록된 사원의 수
SELECT	COUNT(*)	--	20명
FROM EMPLOYEE
...
SELECT	PHONE	--	23행
FROM EMPLOYEE;
-- EMPLOYEE 테이블의 전화번호가 등록되어있지 않은 사원의 수
SELECT	COUNT(*) - COUNT(PHONE)	--	3명
FROM EMPLOYEE;
