...
/*3*/SELECT DEPT_CODE, SUM(SALARY)	-- SUM : 그룹함수,
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DEPT_CODE;	-- 같은 부서코드끼리 그룹을 지음
...
SELECT
	DEPT_CODE,
	COUNT(*)								-- 4.조회 결과에 포함 될 컬럼 지정
FROM EMPLOYEE							-- 1.조회할 테이블
WHERE BONUS IS NOT NULL		-- 2.조회할 행의 조건
GROUP BY DEPT_CODE				-- 3.WHERE 결과에서 그룹 구분
ORDER BY DEPT_CODE ASC;		-- 5.조회결과 정렬
...
SELECT  DEPT_CODE "부서", FLOOR(AVG(SALARY)) "평균급여"
FROM EMPLOYEE
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE	-- 1) 부서별 코드 그룹 후 2) 직급별 그룹
ORDER BY DEPT_CODE ASC, 3 DESC;
...
--> 개인 급여가 350만 이상인 사람들의 부서별 평균 (잘못됨)
SELECT
	DEPT_CODE,
	FLOOR(AVG(SALARY))
FROM EMPLOYEE
...
ORDER BY DEPT_CODE ASC;
--> DEPT_CODE 기준으로 만들어진 그룸(총 7개) 중에서
-- 그룹별 급여평균이 350만 이상인 그룹만 조회
...
-- 4행 조회해서 0,0,0은 조회결과에 포함 안됨
SELECT
	JOB_CODE,
	COUNT(BONUS)
	--> 해당 컬럼에 값이 있는 행이 몇개인이 카운트
	--> NULL이면 카운트에서 제외
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC;
-- 7개 그룹 모두 표시됨
-- 통계적으로 보고싶을때 2번째 방법이 더 좋음
...
INTERSECT
-- 급여가 400만 이상인 사원의 이름, 부서코드, 급여 조회
SELECT
	EMP_NAME,
	DEPT_CODE,
	SALARY
