...
-- 주의 : 혼자서 COMMIT, ROLLBACK 하지 말것! 결과가 달라짐 0-0
...
SELECT * FROM EMP_01;
--> 구문 실행시 24행은 나옴
--> 테이블 가서 보면 아직 COMMIT 안해서 안보이나봄


---------------------------------------
/***************** 정리 ***************************/
INSERT INTO 테이블명
VALUES (모든 컬럼에 추가할 모든 값);
...
-- EMP_EMPLOYEE2 테이블에서 BONUS를 받지 않는 사원의 
-- BONUS를 0.1로 변경
...
-- 테이블에서 지정하는 조건의 값이 존재하면 UPDATE == 키값 중복시 VALUE 수정
-- 조건의 값이 없으면 INSERT됨
...
VALUES (999, '곽두원', '561016-1234567', 'kwack_dw@kh.or.kr',
        ...
        SYSDATE, NULL, DEFAULT); -- 5행됨
       
SELECT * FROM EMP_M01; 
SELECT * FROM EMP_M02;
		-- EMP_M02는
		-- MEP_M01과 4개의 행이 중복되고 개의 행이 다름

UPDATE EMP_M02 SET SALARY = 0;

SELECT * FROM EMP_M02;

MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) -- 중복확인용 KEY
WHEN MATCHED THEN	-- 중복 확인용 KEY가 일치한다면
UPDATE SET	-- 수정(UPDATE)
...
SELECT * FROM EMP_M01 ORDER BY JOB_CODE; 
