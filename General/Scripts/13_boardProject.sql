...
UPDATE "MEMBER"
SET MEMBER_NICKNAME = 'HOST'
WHERE MEMBER_NO = 1;
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'member02@kh.or.kr', '$2a$10$KzFKEvO4C65xBTetZDV8QufZvhQnIGU0SE5ZEaZo0T9SrdYS5oFMC',
	  '호랑이', '01012341234', NULL, NULL,
	   DEFAULT, DEFAULT, DEFAULT);
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'member03@kh.or.kr', '$2a$10$KzFKEvO4C65xBTetZDV8QufZvhQnIGU0SE5ZEaZo0T9SrdYS5oFMC',
	  '지레프', '01012341234', NULL, NULL,
	   DEFAULT, DEFAULT, DEFAULT);
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'member04@kh.or.kr', '$2a$10$KzFKEvO4C65xBTetZDV8QufZvhQnIGU0SE5ZEaZo0T9SrdYS5oFMC',
	  '코끼리', '01012341234', NULL, NULL,
	   DEFAULT, DEFAULT, DEFAULT);
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'member05@kh.or.kr', '$2a$10$KzFKEvO4C65xBTetZDV8QufZvhQnIGU0SE5ZEaZo0T9SrdYS5oFMC',
	  '다람지', '01012341234', NULL, NULL,
	   DEFAULT, DEFAULT, DEFAULT);
 ...
 *   작성자 닉네임 : MEMBER 테이블
 *   댓글 수 : COMMENT 테이블에서 BOARD_NO별 댓글 개수 COUNT(*)
 *   좋아요 개수 : BOARD_LIKE 테이블에서 BOARD_NO 별 댓글 개수? COUNT(*)
 * 
 * - 작성일은 몇 초/분/시간 전 또는 YYYY-MM-DD 형식으로 조회
 * 
 * - 가장 최근 글이 제일 처음 조회
 *   (SEQUENCE 번호로 만들어진 BOARD_NO를 이용)
 * */

--	ROW_NUMBER() OVER (ORDER BY BOARD_NO ASC) "RNUM",
 --> BOARD_NO 오름차준으로 정렬 후 조회된 행의 번호를 지정
 --  + 해당컬럼의 별칭 "RNUM" 지정
 --> 연속되지 않은 PK 대신 연속된 숫자를 만들려고 사용

-- 상관 서브쿼리 해석순서
-- 1) 메인쿼리 1행 해석
-- 2) 서브쿼리에서 메인쿼리 1행 조회결과를 이용
-- 3) 다시 메인쿼리 해석

-- 시간 - 시간 : 결과값이 몇일 차이나는지로 나옴
-- SELECT CURRENT_DATE - TO_DATE('2024-10-01 22:36:00', 'YYYY-MM-DD HH24:MI:SS') FROM DUAL;
...
SELECT
	...
	CASE
		WHEN CURRENT_DATE - BOARD_WRITE_DATE < 1/24/60 -- 1분 미만일 경우
		THEN FLOOR( (CURRENT_DATE - BOARD_WRITE_DATE) *60 *60 *24 ) || '초 전'
		
		WHEN CURRENT_DATE - BOARD_WRITE_DATE < 1/24 -- 1시간 미만일 경우
		THEN FLOOR( (CURRENT_DATE - BOARD_WRITE_DATE) *60 *24 ) || '분 전'
		
		WHEN CURRENT_DATE - BOARD_WRITE_DATE < 1 -- 하루 미만일 경우
...
AND		BOARD_CODE = 1		-- 게시판 종류 (1:공지, 2:자유, 3:정보)
ORDER BY RNUM DESC;
...
SELECT 
	...
	(SELECT COUNT(*) 
	 FROM BOARD_LIKE L
	 WHERE L.BOARD_NO = 2010) AS LIKE_COUNT,
	 
	(SELECT IMG_PATH || IMG_RENAME
	 FROM BOARD_IMG I
	 WHERE I.BOARD_NO = 2010 AND IMG_ORDER = 0) AS THUMBNAIL
...
-------------------------------------------------------
/* 댓글 상세조회 */
SELECT LEVEL, COMMENT_NO, PARENT_COMMENT_NO, COMMENT_CONTENT, M.MEMBER_NICKNAME
FROM "COMMENT" C
JOIN "MEMBER" M ON (C.MEMBER_NO = M.MEMBER_NO)
WHERE BOARD_NO = 2020
-- 계층형 쿼리 코드 추가
-- PARENT_COMMENT_NO 값이 NULL인 행이 계층의 시작
-- == LEVEL 1
START WITH PARENT_COMMENT_NO IS NULL
-- PARENT_COMMENT_NO 값이 COMMENT_NO와 같은 행을
-- 자식으로 지정하여 연결
CONNECT BY PRIOR COMMENT_NO = PARENT_COMMENT_NO
-- 형제들(같은레벨) 간의 정렬 순서를 COMMENT_NO 로 정렬
...
VALUES(
	...
	2020,
	CEIL ( DBMS_RANDOM.VALUE(3024, 3109) )
);

--     부모댓글번호 : 3001 ~ 3024
-- 1차 자식댓글 번호 : 3025 ~ 3110
-- 2차 자식댓글 번호 : 3111 ~ 3140
