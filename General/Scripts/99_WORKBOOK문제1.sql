...
SELECT
	PROFESSOR_NAME 교수이름,
	SUBSTR(PROFESSOR_SSN,1,6) 앞번호,
	CASE
		WHEN SUBSTR(PROFESSOR_SSN,1,2)<50
		THEN FLOOR((CURRENT_DATE - TO_DATE(SUBSTR(PROFESSOR_SSN,1,6), 'YYMMDD'))/365)+100
		
		ELSE FLOOR((CURRENT_DATE - TO_DATE(SUBSTR(PROFESSOR_SSN,1,6), 'RRMMDD'))/365)
	END 나이
FROM TB_PROFESSOR
...
SELECT
	STUDENT_NAME
FROM TB_STUDENT
WHERE (TO_CHAR(ENTRANCE_DATE,'RRRR') - TO_CHAR(TO_DATE(SUBSTR(STUDENT_SSN,1,6), 'RRMMDD'), 'YYYY'))> 19;
