...
CREATE TABLE "CHAT_ROOM" (
	"GROUP_NO"	NUMBER		NOT NULL,
	"MEMBER_NO"	NUMBER		NOT NULL,
	"CHAT_CONTECT_DATE"	DATE	DEFAULT CURRENT_DATE	NOT NULL
);

COMMENT ON COLUMN "CHAT_ROOM"."GROUP_NO" IS '모임 번호';

COMMENT ON COLUMN "CHAT_ROOM"."MEMBER_NO" IS '회원 번호';

COMMENT ON COLUMN "CHAT_ROOM"."CHAT_CONTECT_DATE" IS '마지막 메시지를 읽은 시간';
...
ALTER TABLE "CHAT_ROOM" ADD CONSTRAINT "FK_GROUP_TO_CHAT_ROOM_1" FOREIGN KEY (
	"GROUP_NO"
)
REFERENCES "GROUP" (
	"GROUP_NO"
...
CREATE TABLE "CHAT" (
	...
	"CHAT_CONTENT"	NVARCHAR2(1000)		NOT NULL,
	"CHAT_WRITE_DATE"	DATE	DEFAULT CURRENT_DATE	NOT NULL,
	"CHAT_COUNT"	NUMBER	DEFAULT 1	NOT NULL,
	"MEMBER_NO"	NUMBER		NOT NULL,
	"GROUP_NO"	NUMBER		NOT NULL
);
...
COMMENT ON COLUMN "CHAT"."CHAT_COUNT" IS '채팅 읽은 사람 수';
...
SELECT GROUP_NO FROM "MEMBER_GROUP" WHERE MEMBER_NO = 20;
SELECT MEMBER_NO FROM "MEMBER_GROUP" WHERE GROUP_NO = 1;
...
SELECT
    C.GROUP_NO,
    G.GROUP_NAME,
    COUNT(MG.MEMBER_NO) AS MEMBER_GROUP_COUNT,
    GREATEST(
        COUNT(MG.MEMBER_NO) - COALESCE(MAX(C.CHAT_COUNT), 0),
        0
    ) AS UNREAD_CHAT_COUNT
FROM CHAT C
LEFT JOIN "GROUP" G ON G.GROUP_NO = C.GROUP_NO
LEFT JOIN MEMBER_GROUP MG ON MG.GROUP_NO = C.GROUP_NO
WHERE C.MEMBER_NO = 1  -- 조회할 회원 번호를 여기에 바인딩
GROUP BY C.GROUP_NO, G.GROUP_NAME
ORDER BY MEMBER_GROUP_COUNT;
...
SELECT NVL(GROUP_NO OVER (ORDER BY CHAT_CONTECT_DATE DESC), -1)
FROM CHAT_ROOM
WHERE MEMBER_NO = 1;

SELECT DECODE(GROUP_NO, NULL, -1, 0, -1, GROUP_NO) AS "GROUP_NO"
FROM CHAT_ROOM
WHERE MEMBER_NO = 4
ORDER BY CHAT_CONTECT_DATE DESC 
FETCH FIRST 1 ROWS ONLY;

SELECT NVL(GROUP_NO, -1) AS "GROUP_NO"
FROM (
    SELECT GROUP_NO
    FROM CHAT_ROOM
    WHERE MEMBER_NO = 4
...
SELECT
    ...
    CHAT_CONTECT_DATE
FROM CHAT C
LEFT JOIN "CHAT_ROOM" CR ON C.GROUP_NO = CR.GROUP_NO
WHERE C.MEMBER_NO = 20
AND   C.GROUP_NO = 1;
GROUP BY C.GROUP_NO;
...
SELECT 
    ...
    (SELECT GROUP_NO
		...
		FETCH FIRST 1 ROWS ONLY)
FROM "MEMBER" M
LEFT JOIN "MEMBER_GROUP" MG ON M.MEMBER_NO = MG.MEMBER_NO 
LEFT JOIN "GROUP" G ON MG.GROUP_NO = G.GROUP_NO
WHERE M.MEMBER_NO = 20;
...
SELECT 
	...
	CHAT_COUNT,
	MEMBER_NO,
	(SELECT MEMBER_NICKNAME FROM "MEMBER" M WHERE C.MEMBER_NO = M.MEMBER_NO),
	(SELECT MEMBER_IMG FROM "MEMBER" M WHERE C.MEMBER_NO = M.MEMBER_NO),
	GROUP_NO 
FROM CHAT C
WHERE GROUP_NO= 1;
...
SELECT
    ...
    (
		...
		WHERE MG2.GROUP_NO = MG.GROUP_NO )
		- 
		(SELECT 
			COUNT(*)
		FROM CHAT C
...
AND CHAT_WRITE_DATE > 
	...
	WHERE
		MEMBER_NO = 27
	AND GROUP_NO = 1, TO_DATE('1970-01-01', 'YYYY-MM-DD')));
